...
if strcmp(ext, '.nii') && exist(fname, 'file')
  ...
  fid = fopen(fname, 'w'); % need w+ for debug code below
end

fwrite(fid, h.sizeof_hdr(1), 'int32');

% if strcmp(fname, fullfile(pwd, [basename ext]))
%   fprintf('match\n');
% else
%   fprintf('not match\n');
%   double(fname)
%   double(fullfile(pwd, [basename ext]))
% end
...
% fopen(fid)
% unix(sprintf('ls -al %s',fopen(fid)));
% wd=pwd
% !ls
% cd(fileparts(fopen(fid)))
% pwd
% !ls -al
% unix(sprintf('ls -al %s',fopen(fid)));
% unix(sprintf('ls -al %s',fileparts(fopen(fid))));
% cd(wd)
% fprintf('--\n');
...
if isfield(h, 'intent_q1')
  fwrite(fid, h.intent_q1(1), 'int16');
  fwrite(fid, h.intent_q2(1), 'int16');
else
