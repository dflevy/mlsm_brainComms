function [h, newfname] = read_nifti_hdr(fname)
...
if ~exist(fname, 'file')
  error('Image file %s not found.', fname);
end

[pathstr, name, ext] = fileparts(fname);

if strcmp(ext, '.gz')
  newfname = [tempname '.nii'];%[mytempname '.nii'];
  if ~ispc
      unix_ec(sprintf('gunzip -c %s > %s', fname, newfname));
  else
      [temp_path, temp_name, temp_ext] = fileparts(newfname);
      gunzip(fname, temp_path);
      movefile(fullfile(temp_path, name), fullfile(temp_path, [temp_name temp_ext]));
  end
  need_delete = newfname;

  [pathstr, name, ext] = fileparts(newfname);
else
  newfname = fname;
end
...
fseek(fid, -4, 'cof'); % fmristat
h.intent_q1 = fread(fid, 1, 'int16'); % fmristat
h.intent_q2 = fread(fid, 1, 'int16'); % fmristat
...
if nargout < 2 & ~isempty(need_delete)
  delete(need_delete);
end
